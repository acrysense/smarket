...
(function (root, factory) {
    ...
    var document = window.document,
        ...
        renderTopButtons = function (opts) {
            let monthDate = moment(opts.calendar[0]);
            let enablePrev = opts.minDate ? moment(opts.minDate).isBefore(monthDate.startOf('month')) : true;
            let enableNext = opts.maxDate ? moment(opts.maxDate).isAfter(monthDate.endOf('month')) : true;

            return '<div class="lightpick__toolbar">'
                + ''
                + '<button type="button" class="lightpick__previous-action' + (enablePrev ? '' : ' disabled') + '">' + opts.locale.buttons.prev + '</button>'
                + '<button type="button" class="lightpick__next-action' + (enableNext ? '' : ' disabled') + '">' + opts.locale.buttons.next + '</button>'
        ...
        renderMonthsList = function (date, opts) {
            ...
            for (var idx = 0; idx < 12; idx++) {
                ...
                option.text = firstLetter(d.toDate().toLocaleString(opts.lang, {month: 'long'}));
